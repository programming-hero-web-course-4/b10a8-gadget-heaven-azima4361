...
const Navbar = ({categories=[]}) => {
  ...
  const categoryList = categories.map((category) => category.category_name);
    return (
        <div className={`navbar  text-white mx-auto  px-20 mx-w-[1440px] rounded-xl w-11/12 ${url ==='/' || categoryList.some((name)=> url=== `/category/${name}`) ? " text-white": "text-black py-4"}`}>
  <div className="navbar-start ">
    ...
    <Link to='/' className={`text-3xl font-bold ${url===`/`|| categoryList.some((name)=>url===`/category/${name}`)?"text-white":"text-black"}`}>Gadget Heaven</Link>
  </div>
